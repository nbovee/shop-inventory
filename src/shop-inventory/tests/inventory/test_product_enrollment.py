...
def test_enrollment_step_1_select_location(client, admin_user, location):
    ...
    response = client.post(reverse("inventory:add_item_to_location"), {
...
def test_enrollment_step_2_scan_unknown_barcode(client, admin_user, location):
    ...
    client.post(reverse("inventory:add_item_to_location"), {
        "action": "select_location",
        "location_id": location.id
    })
...
def test_enrollment_complete_workflow(client, admin_user, location):
    ...
    # Final verification
    assert Product.objects.filter(name="Complete Test Item", barcode=new_barcode).exists()
    product = Product.objects.get(name="Complete Test Item", barcode=new_barcode)
    assert Inventory.objects.filter(product=product, location=location, quantity=10).exists()
